...
const AvatarComponent = ({ organizer, withName, size }: TAvatar) => {
        ...
        withName ?
            <div className="flex items-center gap-1 pt-1 md:pt-2">
                <Image
                    ...
                    alt="avatar image"
                />
                <p className={`text-textBlack ${size === "large" ? "text-sm md:text-base font-medium" : "text-xs md:text-sm"}`}>{organizer?.name}</p>
            </div>
            :
            <Image
                width={40}
